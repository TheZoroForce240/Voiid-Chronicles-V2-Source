...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var line:FlxSprite = new FlxSprite(640, 0).makeGraphic(1, 720);
		//add(line);
		...
		titleText = new FlxSprite(175, FlxG.height * 0.8);
		titleText.frames = Paths.getSparrowAtlas('title/titleEnter' + (FlxG.random.bool(1) ? '-wawa' : ''));
		titleText.animation.addByPrefix('idle', "Press Enter to Begin", 24);
		titleText.animation.addByPrefix('press', "ENTER PRESSED", 24);
		titleText.antialiasing = true;
		titleText.scale.set(1.3, 1.3);
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			skippedIntro = true;
			if (logoBl != null)
				FlxTween.tween(logoBl, {y: -100}, 1.5, {ease:FlxEase.cubeInOut, startDelay: 0.5});
		}
	}
}
