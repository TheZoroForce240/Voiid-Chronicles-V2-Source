...
class UISkinSelect extends MusicBeatSubstate
{
    ...
    public function new()
    {
        ...
        ui_Settings = loadConfig(CoolUtil.coolTextFile(Paths.txt("ui skins/" + utilities.Options.getData("uiSkin") + "/config")));
    ...
    function create_Arrows(?new_Key_Count = 4)
    {
		...
		for (i in 0...key_Count)
        {
            ...
            babyArrow.antialiasing = ui_Settings.get("aa") == "true";

			babyArrow.setGraphicSize(Std.int((babyArrow.width * Std.parseFloat(ui_Settings.get("globalScale"))) * (Std.parseFloat(ui_Settings.get("noteScale")) - (Std.parseFloat(mania_size[4 - 1])))));
        ...
        for(i in 0...rating_List.length)
        {
            ...
            rating.setGraphicSize(Std.int(rating.width * Std.parseFloat(ui_Settings.get("globalScale")) * Std.parseFloat(ui_Settings.get("ratingScale"))));
            rating.antialiasing = ui_Settings.get("aa") == "true";
            rating.updateHitbox();
        ...
        combo.setGraphicSize(Std.int(combo.width * Std.parseFloat(ui_Settings.get("globalScale")) * Std.parseFloat(ui_Settings.get("ratingScale"))));
        combo.antialiasing = ui_Settings.get("aa") == "true";
        combo.updateHitbox();
        ...
        for(i in 0...10)
        {
            ...
            number.setGraphicSize(Std.int(number.width * Std.parseFloat(ui_Settings.get("comboScale"))));
			number.antialiasing = ui_Settings.get("aa") == "true";
			number.updateHitbox();
