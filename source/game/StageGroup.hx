...
class StageGroup extends FlxGroup
{
                        ...
                        for(Object in stage_Data.objects)
                        {
                            ...
                            if(Object.is_Animated)
                            {
                                ...
                                for(Animation in Object.animations)
                                {
                                    ...
                                    if(Animation.name.contains("beatHit"))
                                    {
                                        var nums = ["1","2","3","4","8"];
                                        var lastChar = Animation.name.charAt(Animation.name.length-1);
                                        if (nums.contains(lastChar))
                                        {
                                            beatsPerBop = Std.parseInt(lastChar); //if last character is a number
                                            trace(beatsPerBop);
                                        }
    ...
    public function setCharOffsets(?p1:Character, ?gf:Character, ?p2:Character):Void
    {
        ...
        //p1.shader = colorSwap.shader;
        //gf.shader = colorSwap.shader;
        //p2.shader = colorSwap.shader;
        ...
        if(p1.otherCharacters != null)
        {
            for(character in p1.otherCharacters)
            {
                ...
                //character.shader = colorSwap.shader;
            }
        }
    ...
    public function beatHit()
    {
        if(utilities.Options.getData("animatedBGs"))
        {
            ...
            {
                if (PlayState.instance != null && FlxG.state == PlayState.instance && beatsPerBop != 1)
                {
                    if (PlayState.instance.curBeat % beatsPerBop == 0)
                        sprite.animation.play("beatHit"+beatsPerBop, true);
                }
                else 
