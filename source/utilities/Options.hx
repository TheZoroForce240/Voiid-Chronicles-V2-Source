...
class Options
{
    ...
    public static function init()
    {
        ...
        createSaveFullPath("progress", "voiid_chronicles-progress");
    ...
    public static function fixBinds()
    {
        ...
        if(getData("controllerBinds", "binds") == null)
            setData(NoteVariables.Default_Controller_Binds, "controllerBinds", "binds");
        else
        {
            var bindArray:Array<Dynamic> = getData("controllerBinds", "binds");

            if(bindArray.length < NoteVariables.Default_Controller_Binds.length)
            {
                for(i in Std.int(bindArray.length - 1)...NoteVariables.Default_Controller_Binds.length)
                {
                    bindArray[i] = NoteVariables.Default_Controller_Binds[i];
                }
