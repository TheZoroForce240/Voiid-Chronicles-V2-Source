...
class FlxMouse extends FlxPointer implements IFlxInputManager
{
	...
	var _prevGameMouseX:Float = 0.0;
	var _prevGameMouseY:Float = 0.0;
	...
	function update():Void
	{
		...
		if (gamepad != null)
		{
			if (gamepad.analog.justMoved.RIGHT_STICK)
			{
				usingController = true;
			}
			if (usingController)
			{
				//trace(gamepad.analog.value.RIGHT_STICK_X);
				//trace(gamepad.analog.value.RIGHT_STICK_Y);
				_gameMouseX += gamepad.analog.value.RIGHT_STICK_X*FlxG.elapsed*1000;
				_gameMouseY += gamepad.analog.value.RIGHT_STICK_Y*FlxG.elapsed*1000;
			}
		}

		if (FlxG.game.mouseX != _prevGameMouseX || FlxG.game.mouseY != _prevGameMouseY) //Mouse just moved
		{
			usingController = false;
		}
		_prevGameMouseX = FlxG.game.mouseX;
		_prevGameMouseY = FlxG.game.mouseY;

		//if (justMoved)
			//usingController = false;

		if (usingController)
			setGlobalScreenPositionUnsafe(_gameMouseX, _gameMouseY);
		else
		...
		if (visible)
		{
			cursorContainer.x = _gameMouseX;
			cursorContainer.y = _gameMouseY;
		}
		#end
