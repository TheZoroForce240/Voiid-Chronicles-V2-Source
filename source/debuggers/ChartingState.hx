...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function createStageAndCharacters()
	{
		...
		camBG.follow(camFollow, LOCKON, 0.04 * (60 / Main.display.currentFPS));
	...
	var healthBarTypeText:FlxUIInputText;
	...
	function addNoteUI():Void
	{
		...
		healthBarTypeText = new FlxUIInputText(10, 200, 70, _song.healthBar_Skin, 8);
		...
		var hpBarLabel = new FlxText(12 + healthBarTypeText.width, healthBarTypeText.y, 0, "Health Bar Skin", 9);
		...
		tab_group_note.add(hpBarLabel);

		tab_group_note.add(healthBarTypeText);
	...
	override function update(elapsed:Float)
	{
		...
		_song.healthBar_Skin = healthBarTypeText.text;
				...
				if (FlxG.sound.music.playing)
				{
					...
					//{
						if(!claps.contains(note) && note.shouldHit 
							&& note.strumTime <= Conductor.songPosition && note.strumTime >= Conductor.songPosition - Conductor.safeZoneOffset) //check if should be hit
						{
							claps.push(note);
							
							if(note.rawNoteData % (_song.keyCount + _song.playerKeyCount) < _song.playerKeyCount && _song.notes[curSection].mustHitSection || note.rawNoteData % (_song.keyCount + _song.playerKeyCount) >= _song.keyCount && !_song.notes[curSection].mustHitSection)
							{
								if (hitsounds)
									FlxG.sound.play(Paths.sound('CLAP', 'preload'));

								if(bf.otherCharacters == null || bf.otherCharacters.length - 1 < note.character)
								{
									bf.playAnim(NoteVariables.Character_Animation_Arrays[_song.playerKeyCount - 1][Std.int(Math.abs(note.noteData))], true);
									bf.holdTimer = -note.sustainLength*0.001;
								}
								else
								{
									if(note.characters.length <= 1)
									{
										bf.otherCharacters[note.character].playAnim(NoteVariables.Character_Animation_Arrays[_song.playerKeyCount - 1][Std.int(Math.abs(note.noteData))], true);
										bf.otherCharacters[note.character].holdTimer = -note.sustainLength*0.001;
									}
									else
									{
										for(character in note.characters)
										{
											if(bf.otherCharacters.length - 1 >= character)
											{
												bf.otherCharacters[character].playAnim(NoteVariables.Character_Animation_Arrays[_song.playerKeyCount - 1][Std.int(Math.abs(note.noteData))], true);
												bf.otherCharacters[character].holdTimer = -note.sustainLength*0.001;
											}
							...
							else
							{
								...
								{
									dad.playAnim(NoteVariables.Character_Animation_Arrays[_song.keyCount - 1][Std.int(Math.abs(note.noteData))] + altAnim, true);
									dad.holdTimer = -note.sustainLength*0.001;
								}
								else
								{
									if(note.characters.length <= 1)
									{
										dad.otherCharacters[note.character].playAnim(NoteVariables.Character_Animation_Arrays[_song.keyCount - 1][Std.int(Math.abs(note.noteData))] + altAnim, true);
										dad.otherCharacters[note.character].holdTimer = -note.sustainLength*0.001;
									}
									else
									{
										for(character in note.characters)
										{
											...
											{
												dad.otherCharacters[character].playAnim(NoteVariables.Character_Animation_Arrays[_song.keyCount - 1][Std.int(Math.abs(note.noteData))] + altAnim, true);
												dad.otherCharacters[character].holdTimer = -note.sustainLength*0.001;
											}
										}
									}
								}
		...
		bpmTxt.text = (
			...
			+ zoom_level
			+ "\n"
			+ (sectionIsBroken ? "SECTION HAS BROKEN NOTES!!!!" : "")
			+ "\n"
			+ (sectionIsBroken ? "Press 1 to try to fix it" : "")
			+ "\n"
		);
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var note:Note = new Note(daStrumTime, goodNoteInfo, null, false, char, daType, _song, chars, mustPress, true);
			note.sustainLength = daSus;

			var scaleY:Int = 40;
			if (PlayState.instance.uiSkin.arrow_Configs.get(daType).exists("charterScaleY"))
				scaleY = Std.parseInt(PlayState.instance.uiSkin.arrow_Configs.get(daType).get("charterScaleY"));

			note.setGraphicSize((PlayState.instance.uiSkin.arrow_Configs.get(daType).exists("charterScaleX") ? Std.parseInt(PlayState.instance.uiSkin.arrow_Configs.get(daType).get("charterScaleX")) : 0), scaleY);
			note.updateHitbox();


			var yOff:Float = 0;
			if (PlayState.instance.uiSkin.arrow_Configs.get(daType).exists("charterY"))
				yOff = Std.parseFloat(PlayState.instance.uiSkin.arrow_Configs.get(daType).get("charterY"));

			note.x = Math.floor((daNoteInfo + 1) * GRID_SIZE) + (GRID_SIZE/2) - (note.width/2); //+ Std.parseFloat(PlayState.instance.arrow_Configs.get(daType)[1]);
			note.y = Math.floor(getYfromStrum((daStrumTime - sectionStartTime())) + yOff);
			...
			if(doFunnyNumbers)
			{
				...
				curRenderedIdTexts.add(id);
			}

			if (daSus > 0)
			{
				var susOff:Float = 0;
				if (PlayState.instance.uiSkin.arrow_Configs.get(daType).exists("charterX"))
					susOff = Std.parseFloat(PlayState.instance.uiSkin.arrow_Configs.get(daType).get("charterX"));

				var sustainVis:FlxSprite = new FlxSprite(note.x + (GRID_SIZE / 2) - susOff,
					note.y + GRID_SIZE - yOff).makeGraphic(8, Math.floor(FlxMath.remapToRange(daSus, 0, Conductor.stepCrochet * Conductor.stepsPerSection, 0, gridBG.height)));
